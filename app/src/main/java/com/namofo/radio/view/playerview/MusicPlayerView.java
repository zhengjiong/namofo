...
*/
package com.namofo.radio.view.playerview;
...
public class MusicPlayerView extends View implements OnPlayPauseToggleListener {
     ...
     * zh
     */
    private boolean mProgressVisibility = false;//不显示进度条和文字
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        //mButtonRadius = mWidth / 8.0f;zhengjiong
        mButtonRadius = getResources().getDimensionPixelSize(R.dimen.play_button_width);
    ...
    public void setCoverURL(int resId) {
        Glide.with(getContext()).load(resId).into(target);
    }
    ...
    private SimpleTarget target = new SimpleTarget() {
        ...
        public void onResourceReady(Object resource, GlideAnimation glideAnimation) {
            if (resource instanceof GlideBitmapDrawable) {
                mBitmapCover = ((GlideBitmapDrawable) resource).getBitmap();
                createShader();
                postInvalidate();
